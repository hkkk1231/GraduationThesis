...
def fix_existing_note():
    ...
    try:
        ...
        # 找到需要修复的笔记
        note_key = "WC7G53SC"  # 选择一个代码创建的笔记进行修复
        cursor.execute("""
            SELECT itemID, note FROM itemNotes WHERE itemID = (
        ...
        item_id, original_note = result
        
        # 创建正确的HTML结构
        fixed_note = f"""<div class="zotero-note znv1"><div data-schema-version="9">{original_note[25:]}</div></div>"""
        
        print(f"原始笔记长度: {len(original_note)}")
        print(f"修复后笔记长度: {len(fixed_note)}")
        print(f"修复后笔记预览: {fixed_note[:200]}...")
...
def create_new_note_with_correct_structure():
    """创建具有正确结构的新笔记"""
    db_path = "C:\\Users\\28480\\Zotero\\zotero.sqlite"
...
if __name__ == "__main__":
    ...
    print("\n2. 创建新的正确格式笔记")
    create_new_note_with_correct_structure()
